export async function onRequest(context) {
  ...
  if (context.request.method === 'POST') {
    try {
      ...
      // If an amount is provided (e.g. token usage), add it to a running sum
      if (typeof body.amount === 'number' && !isNaN(body.amount)) {
        const sumKey = `sum:${type}${value ? ':' + value : ''}`;
        const sumExisting = await kv.get(sumKey);
        const sum = (sumExisting ? parseInt(sumExisting) : 0) + body.amount;
        await kv.put(sumKey, String(sum));
      }
      // Log recent events (keep last 100)
      const recentKey = 'recent:events';
      const recentRaw = await kv.get(recentKey);
      const recent = recentRaw ? JSON.parse(recentRaw) : [];
      recent.unshift({ type, value, amount: body.amount, ts: new Date().toISOString() });
  ...
  if (context.request.method === 'GET' && action === 'read') {
    try {
      ...
      for (const key of list.keys) {
        ...
        } else if (key.name.startsWith('sum:')) {
          const val = await kv.get(key.name);
          sums[key.name.replace('sum:', '')] = parseInt(val) || 0;
        }
      }
      const recentRaw = await kv.get('recent:events');
      const recent = recentRaw ? JSON.parse(recentRaw) : [];
      return new Response(JSON.stringify({ counts, sums, recent }), { headers: corsHeaders });
    } catch (err) {
      return new Response(JSON.stringify({ error: err.message }), { status: 500, headers: corsHeaders });
    }
  }
